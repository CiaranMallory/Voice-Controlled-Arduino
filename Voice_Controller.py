...
def action(command):
    Arduino_Serial = serial.Serial('com13',9600)
    print Arduino_Serial.readline() # read the serial data and print it as a line

    while 1: # Infinite loop
        
        if "Turn on LED" in command: 
            Arduino_Serial.write('1')
            voice('LED ON')
           
        
        if "Turn off LED" in command:
